...
int send_file(const char *file, off_t start, off_t end, int client_sock, int chunk_num) {
    ...
    while (bytes_to_send > 0) {
        ...
        size_t packet_size = 50 + bytes_read;
        ...
        memcpy(packet, header, 50);
        memcpy(packet + 50, buffer, bytes_read);
        printf("%s\n",packet);
...
void* handle_connection(void* arg) {
    while(1){
        ...
        if (received > 0) {
            ...
            else if(strcmp(buffer,"client_check")==0){
                send(client_sock,"File Name?",10,0);
                memset(buffer,0,MAX_LINE_LENGTH);
                int len = recv(client_sock, buffer, sizeof(buffer)-1,0);
                buffer[len] = '\0';
